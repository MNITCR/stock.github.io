$(document).ready(function() {
    ...
    $('#search_input').on('keyup', function() {
        ...
        $('#cat_data tr.cat_row').each(function() {
            // Get the content of the row's name column in lowercase
            var rowData = $(this).find('td:nth-child(2)').text().toLowerCase();
            var rowDataKH = $(this).find('td:nth-child(3)').text().toLowerCase();
            if (rowData.includes(searchText) || rowDataKH.includes(searchText)) {
                $(this).show();
            } else {
                $(this).hide();
            }
        ...
        if ($('#cat_data tr.cat_row:visible').length === 0) {
            $noUserRow.show();
        } else {
            $noUserRow.hide();
        }
